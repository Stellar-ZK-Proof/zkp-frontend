...
import type { PaymentResult } from "@/components/PaymentForm";
  ...
  return (
    <div className="bg-navy-900 rounded-xl p-4">
      ...
      <div className="flex items-center justify-between gap-3">
        <span className="font-mono text-sm text-cyan-400 break-all leading-relaxed">{truncate(value, 16)}</span>
        <button onClick={copy} className="shrink-0 text-slate-600 hover:text-cyan-400 transition-colors">
    ...
    return (
      <div className="glass rounded-3xl p-8 flex flex-col items-center justify-center min-h-96 text-center">
        ...
        <p className="text-slate-600 text-sm max-w-xs leading-relaxed">
          Connect Freighter, fill in the form, and submit to see your ZK settlement receipt here.
        </p>
        <div className="mt-8 w-full max-w-xs space-y-2 text-left">
          {["Connect Freighter wallet", "Generate ZK proof in browser", "Submit & settle on Stellar testnet"].map((s, i) => (
            <div key={i} className="flex items-center gap-2 text-xs text-slate-700 font-mono">
              <span className="w-5 h-5 rounded-full border border-slate-800 flex items-center justify-center text-[10px] shrink-0">{i + 1}</span>
              {s}
            </div>
          ))}
        </div>
    ...
    <div className="glass rounded-3xl p-8 animate-slide-up">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-xs text-slate-500 font-mono mt-0.5">ZK proof · Stellar testnet</p>
        </div>
        <span className="ml-auto px-2 py-0.5 rounded-full bg-emerald-400/10 text-emerald-400 text-xs font-mono capitalize">
          {result.status}
        </span>
      </div>
      ...
      <div className="mt-5 rounded-xl border border-cyan-400/10 bg-cyan-400/5 px-4 py-3 text-xs text-slate-400 leading-relaxed">
        ...
        Amount and recipient are hidden in the commitment. Only the audit key holder can reveal them.
      </div>
